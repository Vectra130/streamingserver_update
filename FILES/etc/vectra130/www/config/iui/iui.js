...
//			console.log("xhr.readyState = " + xhr.readyState);
			if (xhr.readyState == 4)
			{
				if (xhr.status == 200 && !xhr.aborted)
				{
					...
					{
						if(frag.childNodes[i].firstChild==null)
						frag.removeChild(frag.childNodes[i]);
						else if(iui.hasClass(frag.childNodes[i],'toolbar'))
						frag.removeChild(frag.childNodes[i]);
					}
...
//		docNode = page.appendChild(frag.firstChild);
		docNode = page.insertBefore(frag.firstChild, parent);
	page.removeChild(parent);
